...
#### 内容部分
1. 尽可能减少HTTP请求，包括文字、图片、script、CSS等所有从服务器获取的内容
  - 合并请求内容，将所有脚本放在一个文件中处理
  - CSS Sprites：将背景图片整合在一张图片中，用CSS的background-image和background-position属性定位要显示的部分
  - Base64编码图片：使用`data:URL`模式把图片嵌入页面，可以将图片信息放在样式表中缓存。

1. 减少DNS查找
  - 使用DNS缓存

1. 添加Expire/Cache-Control头，使用本地缓存

1. 使用CDN`内容分发网络`



1. Gzip压缩，在文件传输中有重大意义

1. 将css放在页面最上面

1. 将javascript脚本放在页面最下面

1. 避免在css中使用Expressions
CSS Expressions: 1. 页面现实和缩放 2. 页面滚动 3. 移动鼠标

<!-- more -->

页面解析流程
``` 
html -> head -> meta -> title -> style -> link -> script -> body -> div -> img
```
